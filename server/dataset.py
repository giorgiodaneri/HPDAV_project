...
def get_firewall_dataset_filtered():
    """
    Import and filter firewall dataset
    return dataset_firewall, len(dataset_firewall)
    """
    # Define the file path
    file_path = '../MC2-CSVFirewallandIDSlogs/Firewall_global_sampled.csv'
    # Load the first ten rows of the CSV file
    data = pd.read_csv(file_path)
    new_data = pd.DataFrame()
    new_data = data.copy()
